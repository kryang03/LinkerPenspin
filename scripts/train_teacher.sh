#!/bin/bash
# scripts/train_teacher.sh 0 42 tmp                                                   用于测试初始阶段Reward Scale
# nohup scripts/train_teacher.sh 3 42 pose4_100k_cfg2 > pose4_100k_cfg2.txt &                   生成可以关闭终端的训练任务，需要通过kill ID关闭    
...
# 4. 确定命令的GPU参数
# 5. 去linker_hand_hora.py中修改CHECKLIST
...
# test=True 是用来load weight进行测试的 checkpoint=outputs/LinkerHandHora/thresh0_addposenew_nocrash/stage1_nn/best*.pth \
...
train.ppo.output_name=LinkerHandHora/${CACHE} task.env.grasp_cache_name=4pose \
