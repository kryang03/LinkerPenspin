#!/bin/bash
# nohup scripts/train_teacher.sh 0 42 pose3_50k_cfg3 > pose3_50k_cfg3.txt &                   生成可以关闭终端的训练任务，需要通过kill ID关闭    
...
task.env.reward.angvelClipMax=5.0 task.env.reward.angvelPenaltyThres=5.0 \
checkpoint=outputs/LinkerHandHora/pose3_50k_cfg2/stage1_nn/best*.pth \
