...
class ActorCritic(nn.Module):
    def __init__(self, kwargs):
        ...
        if "ends" in self.input_mode and self.student: # TODO 下一阶段要改维度
            policy_input_dim += 18
            self.end_feat_extractor = MLP(units=[6,6,6], input_size=3, with_last_activation=False)

        if self.student: # TODO 下一阶段要改维度
    ...
    def _actor_critic(self, obs_dict):
        ...
        if not self.student:
            ...
            new_priv = fingertip_pos + ((torch.rand(fingertip_pos.shape[0], 3*NUM_FINGERTIP).cuda()) - 0.5) * 0.02
