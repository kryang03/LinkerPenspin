...
class PPO(object):
    ...
    def __init__(self, env, output_dif, full_config):
        ...
        # 初始化 AdamWW 优化器，优化模型的参数
        self.optimizer = torch.optim.AdamWW(self.model.parameters(), self.last_lr, weight_decay=self.weight_decay)
    ...
    def test(self):
        ...
        while True:
            ...
            if self.ppo_config.distill:
                ...
                input_dict = {
                    'obs': self.running_mean_std(obs_dict['obs']), # 标准化后的观察
                    'priv_info': self.priv_mean_std(obs_dict['priv_info']) if self.normalize_priv else obs_dict['priv_info'], # 标准化或原始的特权信息
                    'proprio_hist': obs_dict['proprio_hist'], # 本体感觉历史信息
                    'point_cloud_info': point_cloud, # 标准化或原始的点云信息
                }
            # 调用模型进行推理模式的动作预测
