...
class VecTask(Env):
    ...
    def _allocate_buffers(self):
        ...
        # TODO 这里的obs_buf_lag_history/obs_buf是硬编码的，在改变构型时这里也要改.
        # obs_buf只要了obs_buf_lag_history最后一维度的前42个，也就是只有joint pos和target。一共往前要了三个时间步的obs，维度126
        self.obs_buf = torch.zeros((self.num_envs, 6*NUM_DOF), device=self.device, dtype=torch.float)
        # dim1= 80,是指定history_len
        # dim2= 2* num_hand_dofs + self.num_contacts(FINGERTIP_CNT*5 + 每个手指的net三维力 + FINGERTIP_CNT * (pos=3)
        self.obs_buf_lag_history = torch.zeros((
            self.num_envs, 80, 2*NUM_DOF + CONTACT_DIM + FINGERTIP_CNT*3 if self.config['env']['privInfo']['enable_tactile'] else 2*NUM_DOF,
