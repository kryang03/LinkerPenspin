...
DEBUG_CONFIG = {
    ...
    "hand_dof_targets_rad": [0.1795, -1.2081, -0.3971, -0.5028, -0.1800, -1.0195, -0.8944, -1.1978, 0.0000, -1.2235, -0.5445, -0.4064, -0.0001, -0.9299, -1.2790, -0.9383, -0.0113, -1.3017, -0.4388, -0.3019, -0.4388],
    # !! 修改这里的值来设置物体的初始位置 [x, y, z] (米) !!
    "object_initial_pos": [-0.1124000060558319, 0.03090000070631504, 0.25130000722408295],
    # !! 修改这里的值来设置物体的初始旋转 (四元数) !!
    "object_initial_rot": [0.44620001316070557, -0.5485000014305115, 0.5485000014305115, -0.44620001316070557],
    # !! 新增：修改这里的值来缩放物体大小 (1.0 为原始大小) !!
    "object_scale": 0.3,
    "object_move_axis": "z",#y, z
    "object_move_speed": 0,#-0.000003,#
    # --- 调试参数 ---
...
def main():
    ...
    asset_options_object.disable_gravity = False  # 禁用重力以便更容易控制
    ...
    hand_start_pose.r = quat_from_axis_angle([0, 1, 0], rotate_y_rad) # 绕 X 轴旋转
    ...
    # 新增：物体旋转角度
    object_angle = 0.0
    object_rotate_interval = 0  # 你可以调整旋转速度
    q = DEBUG_CONFIG["object_initial_rot"]
    object_init_quat = gymapi.Quat(q[0], q[1], q[2], q[3])
    # 新增：物体平移参数
    object_move_axis = DEBUG_CONFIG.get("object_move_axis", "x")
    object_move_speed = DEBUG_CONFIG.get("object_move_speed", 0.0)
    object_pos_offset = 0.0
    while not gym.query_viewer_has_closed(viewer):
        ...
        # 新增：每帧沿指定轴缓慢移动
        object_pos_offset += object_move_speed
        for env_id in range(num_envs):
            if object_move_axis == "x":
                object_actor_states[env_id, 0] = object_init_pos[0] + object_pos_offset
                object_actor_states[env_id, 1] = object_init_pos[1]
                object_actor_states[env_id, 2] = object_init_pos[2]
            elif object_move_axis == "y":
                object_actor_states[env_id, 0] = object_init_pos[0]
                object_actor_states[env_id, 1] = object_init_pos[1] + object_pos_offset
                object_actor_states[env_id, 2] = object_init_pos[2]
            else:
                object_actor_states[env_id, 0] = object_init_pos[0]
                object_actor_states[env_id, 1] = object_init_pos[1]
                object_actor_states[env_id, 2] = object_init_pos[2] + object_pos_offset
